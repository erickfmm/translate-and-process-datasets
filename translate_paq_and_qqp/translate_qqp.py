...
from gpu_temp_guard import run_temp_guard_supervisor, load_or_create_workbook
...
def worker_process(
	...
	try:
		...
		while True:
			...
			batch: List[Tuple[int, str, str, List[str]]] = task
...
class MasterCoordinator:
	...
	4. Writes ordered results to a single XLSX file (non-blocking, buffered).
	"""

	XLSX_HEADERS = ['index', 'Q_original', 'POS_original', 'NEGs_original',
					'Q_traducida', 'POS_traducida', 'NEGs_traducidas']
	...
	def run(self) -> None:
		...
		# Prepare XLSX workbook (resume-append if the file already exists)
		self._workbook, self._sheet = load_or_create_workbook(
			self._output_path, self.XLSX_HEADERS, resume_append=self.resume_append
		)

		# Build batches from the dataset
		batches: List[List[Tuple[int, str, str, List[str]]]] = []
		current_batch: List[Tuple[int, str, str, List[str]]] = []
		processed = 0
		...
		for i, data in enumerate(dataset):
			...
			current_batch.append((i, Q_original, POS_original, NEGs_original))
			processed += 1
		...
		print(
			f"Completed. Translated {total_rows} triplets -> {self.output_excel} "
...
# ---------------------------------------------------------------------------
# Single-process mode (used when --workers=1)
# ---------------------------------------------------------------------------
def translate_triplets_single(skip_rows: int,
							  max_rows: Optional[int],
							  output_excel: str,
							  log_file: str,
							  engine_config: Dict[str, Any],
	...
	workbook, sheet = load_or_create_workbook(
		output_path, MasterCoordinator.XLSX_HEADERS, resume_append=resume_append
	)

	# Buffer for reordering and non-blocking writes
...
def build_arg_parser():
	...
	p.add_argument('--flush-interval-seconds', type=float, default=5.0,
				   help='Maximum seconds between XLSX flushes')
	...
	p.add_argument(
		...
		help='Kill the GPU worker when it reaches this temp (C) so VRAM is '
		'freed and the GPU cools. Only active when --device is cuda (default: 80)',
	)
	p.add_argument(
		'--temp-guard-resume', type=int, default=75,
		help='Restart the GPU worker once temp drops to this value (C), '
		'resuming from the last saved row. Only active when --device is cuda (default: 75)',
	)
	p.add_argument(
		'--temp-guard-stop', type=int, default=90,
		help='Permanently abort (kill worker, stop supervising) at this temp (C). '
...
def _run_as_supervisor_or_worker(args, parser) -> bool:
	"""Handle the GPU temperature-guard supervisor lifecycle.

	Returns True if the call was handled (supervisor ran or worker translation
	was dispatched) and main() should return; False if the caller should run
	the translation directly (no guard active).
	"""
	device_str = (args.device or "").lower()
	if not device_str.startswith("cuda"):
		return False

	# When invoked by the supervisor, TRANSLATE_SUPERVISOR_CHILD=1 is set: run
	# the translation directly (appending to any existing output file) and let
	# the supervisor manage kill/restart.
	resume_append = os.environ.get("TRANSLATE_SUPERVISOR_CHILD") == "1"
	skip_rows = args.skip_rows
	if resume_append:
		env_skip = os.environ.get("TRANSLATE_SKIP_ROWS")
		if env_skip is not None:
			skip_rows = int(env_skip)

	engine_config = engine_config_from_args(args)

	if not resume_append:
		# Top-level invocation on CUDA: become the CPU supervisor that spawns,
		# watches, kills and restarts the GPU worker subprocess.
		...
		rc = run_temp_guard_supervisor(
			script_path=os.path.abspath(__file__),
			argv=sys.argv[1:],
			output_xlsx=args.output_excel,
			initial_skip_rows=args.skip_rows,
	...
	# Worker (child) mode: translate, appending to the existing output file.
	if args.workers == 1:
...
def main():
	...
	# GPU temperature-guard supervisor (CUDA only). Handles its own lifecycle.
	if _run_as_supervisor_or_worker(args, parser):
		return
